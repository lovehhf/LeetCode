...
class Solution(object):
    ...
    def videoStitching2(self, clips, T):
        ...
        for c in clips:
            ...
            if dp[c[0]] == -1:
                return -1
            r = dp[c[0]] + 1
            for i in range(c[0] + 1, min(T,c[1] + 1)):
                if dp[i] == -1:
                    dp[i] = r
                else:
                    dp[i] = min(dp[i], r)
    ...
    def videoStitching3(self, clips, T):
        ...
        :rtype: int
        """
        clips=sorted(clips,key=lambda x:x[1],reverse=True)
        print(clips)
        now=0
        ans=0
        while(True):
            ...
            for n in clips:
                if n[0]<=now:
                    ...
                    #flag=True
                    break
            if nowlast==now:return -1
            if now>=T:return ans

    def videoStitching4(self, clips, T):
        """
        :type clips: List[List[int]]
        :type T: int
        :rtype: int
        ...
        while now_at < T:
            ...
            last = None
            while clips:
                clip = heapq.heappop(clips)
                if clip[0] > now_at:
                    heapq.heappush(clips, clip)
                    break
                last = max(last, clip[1])
            if last:
                now_at = last
            else:
                return -1
