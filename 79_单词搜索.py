...
class Solution(object):
    ...
    # 从(i,j)开始 检查是否能找到word
    def dfs(self, board, i, j, word):
        # word所有的字符都找到了
        if len(word) == 0:
            return True

        # print(borad, i, j, word)
        ...
        for i in range(m):
            for j in range(n):
                if board[i][j] == word[0]:
                    ...
                    while queue:
                        x, y, idx, visited = queue.pop(0)
                        for di, dj in ds:
                            dx = x + di
                            dy = y + dj
                            if 0 <= dx < m and 0 <= dy < n and idx < len(word) - 1 and board[dx][dy] == word[
                                idx + 1] and (dx, dy) not in visited:
                                queue.append((dx, dy, idx + 1, visited | {(dx, dy)}))
                                if idx == len(word) - 2:
                                    return True
        return False
